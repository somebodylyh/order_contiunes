...
def main():
    ...
    _, val_loader, test_loader, _ = create_continuous_dataloaders(
        vector_dim=cfg.vector_dim,
        seq_length=cfg.seq_length,
        dependency_window=cfg.dependency_window,
        num_matrices=cfg.num_matrices,
        train_samples=100,  # minimal, not used
    ...
    for name, path in models_to_eval:
        ...
        print("\n--- Validation Set ---")
        val_results = evaluate_all_orders(model, val_loader, device, num_mc_samples=20)
        ...
        print("\n--- Test Set ---")
        test_results = evaluate_all_orders(model, test_loader, device, num_mc_samples=20)
