...
class ContinuousRotationDataset(Dataset):
    ...
    def _pregenerate_samples(self) -> None:
        """Pre-generate all samples for eval mode (batched to avoid OOM)."""
        if self.seed is not None:
            torch.manual_seed(self.seed)
            np.random.seed(self.seed)

        print(f"[INFO] Pre-generating {self.num_samples} {self.init_mode} samples...")
        chunk_size = min(10000, self.num_samples)
        all_vectors = []
        all_init_vectors = []
        ...
        while remaining > 0:
            ...
            all_init_vectors.append(result['init_vectors'])
            remaining -= bs
            print(f"  ... generated {self.num_samples - remaining}/{self.num_samples}")

        self.samples = {
            'vectors': torch.cat(all_vectors, dim=0),
            'init_vectors': torch.cat(all_init_vectors, dim=0)
        }
...
def create_continuous_dataloaders(
    ...
    # Create datasets (all offline pre-generated to prevent memorization)
...
class MemmapDataset(Dataset):
    """Dataset that reads pre-generated data from disk via numpy memmap (near-zero RAM)."""
    ...
    def __init__(self, data_dir: str, split: str, num_chunks: int = 4):
        ...
        self.vectors = self.vectors.reshape(n, L, D)
        self.init_vectors = self.init_vectors.reshape(n, k, D)
        self.num_chunks = num_chunks
        self.n = n
    ...
    def __getitem__(self, idx):
        ...
        init_vecs = torch.from_numpy(self.init_vectors[idx].copy())
...
def create_continuous_dataloaders_from_disk(
    data_dir: str,
    batch_size: int = 64,
    num_workers: int = 4,
    num_chunks: int = 4,
) -> Tuple[DataLoader, DataLoader, DataLoader]:
    """Create train/val/test dataloaders from pre-generated memmap data on disk."""
    train_dataset = MemmapDataset(data_dir, 'train', num_chunks=num_chunks)
    val_dataset = MemmapDataset(data_dir, 'val', num_chunks=num_chunks)
    test_dataset = MemmapDataset(data_dir, 'test', num_chunks=num_chunks)

    print(f"[INFO] Loaded {len(train_dataset)} train, {len(val_dataset)} val, {len(test_dataset)} test from {data_dir}")
